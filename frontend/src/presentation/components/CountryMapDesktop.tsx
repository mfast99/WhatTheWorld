...
export default function CountryMapDesktop({
  ...
  useEffect(() => {
    ...
    const createCustomIcon = (isSelected: boolean = false) => {
      ...
      return L.divIcon({
        html: `
          <div class="pin-marker" style="position: relative; width: 40px; height: 56px; cursor: pointer;">
            <div class="pin-head" style="position: absolute; top: 0; left: 50%; transform: translateX(-50%); width: 40px; height: 40px; background: ${isSelected ? '#0ea5e9' : isDark ? '#1e293b' : '#ffffff'}; border: 2px solid ${isSelected ? '#0284c7' : isDark ? '#475569' : '#cbd5e1'}; border-radius: 50%; box-shadow: ${isSelected ? '0 4px 12px rgba(14, 165, 233, 0.4)' : isDark ? '0 2px 8px rgba(0, 0, 0, 0.6)' : '0 2px 8px rgba(0, 0, 0, 0.15)'}; display: flex; align-items: center; justify-content: center; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);">
              <span style="font-size: 20px; font-weight: 700; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif; color: ${isSelected ? '#ffffff' : isDark ? '#e2e8f0' : '#334155'}; line-height: 1;">i</span>
            </div>
            <div class="pin-needle" style="position: absolute; top: 38px; left: 50%; transform: translateX(-50%); width: 2px; height: 18px; background: ${isSelected ? '#0ea5e9' : isDark ? '#475569' : '#94a3b8'}; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);"></div>
          </div>
          <style>
            .pin-marker:hover .pin-head {
              transform: translateX(-50%) scale(1.15);
              box-shadow: ${isSelected ? '0 6px 16px rgba(14, 165, 233, 0.6)' : isDark ? '0 4px 12px rgba(0, 0, 0, 0.7)' : '0 4px 12px rgba(0, 0, 0, 0.25)'};
            }
  ...
  return (
    <div 
      style={{
        display: 'grid',
        gridTemplateColumns: selectedCountry ? '1fr 384px' : '1fr',
        width: '100%',
        height: '100%',
